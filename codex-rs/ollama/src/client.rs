...
impl OllamaClient {
    ...
    pub async fn try_from_oss_provider(config: &Config) -> io::Result<Self> {
        ...
        Self::build_from_provider(provider).await
    }

    /// Construct a client from an arbitrary provider definition.
    pub async fn try_from_provider_info(provider: &ModelProviderInfo) -> io::Result<Self> {
        Self::build_from_provider(provider).await
    }
    ...
    async fn build_from_provider(provider: &ModelProviderInfo) -> io::Result<Self> {
