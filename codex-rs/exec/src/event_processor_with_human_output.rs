...
impl EventProcessor for EventProcessorWithHumanOutput {
    ...
    fn process_event(&mut self, event: Event) -> CodexStatus {
        ...
        match msg {
            ...
            EventMsg::TokenCount(ev) => {
                ...
                self.last_rate_limits = ev.rate_limits;
            }
    ...
    fn print_final_output(&mut self) {
        ...
        if let Some(snapshot) = &self.last_rate_limits {
            let summaries = summarize_rate_limits(snapshot);
            if !summaries.is_empty() {
                eprintln!("{}", "rate limits".style(self.magenta).style(self.italic));
...
fn summarize_rate_limits(snapshot: &RateLimitSnapshot) -> Vec<String> {
    let mut summaries = Vec::new();

    if let Some(primary) = snapshot.primary.as_ref()
        && let Some(line) = format_rate_limit_line(primary, RateLimitWindowKind::Hours(5))
    {
        summaries.push(line);
    }

    if let Some(secondary) = snapshot.secondary.as_ref()
        && let Some(line) = format_rate_limit_line(secondary, RateLimitWindowKind::Weekly)
    {
        summaries.push(line);
    }
...
fn format_rate_limit_line(
    ...
    let kind = resolve_window_kind(window.window_minutes, fallback_kind);
