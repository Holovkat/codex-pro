...
use crate::exec_events::RateLimitUsage;
...
impl EventProcessorWithJsonOutput {
    ...
    pub fn collect_thread_events(&mut self, event: &Event) -> Vec<ThreadEvent> {
        match &event.msg {
            ...
            EventMsg::TokenCount(ev) => {
                ...
                self.last_rate_limits = ev.rate_limits.clone();
                Vec::new()
            }
    ...
    fn handle_task_complete(&mut self) -> Vec<ThreadEvent> {
        ...
        if let Some(u) = &self.last_total_token_usage {
            usage.input_tokens = u.input_tokens;
            usage.cached_input_tokens = u.cached_input_tokens;
            usage.output_tokens = u.output_tokens;
        }
        usage.rate_limits = self.last_rate_limits.as_ref().map(RateLimitUsage::from);
