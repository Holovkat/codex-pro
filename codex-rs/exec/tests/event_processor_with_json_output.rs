...
use codex_exec::exec_events::RateLimitUsage;
...
#[test]
fn task_complete_includes_rate_limits() {
    let mut ep = EventProcessorWithJsonOutput::new(None);
    ...
    assert!(ep.collect_thread_events(&token_event).is_empty());
    ...
    assert_eq!(
        ...
        vec![ThreadEvent::TurnCompleted(TurnCompletedEvent {
            usage: Usage {
                ...
                output_tokens: 0,
                rate_limits: Some(RateLimitUsage::from(&snapshot)),
            },
        })]
    );
}
