...
pub struct CustomProviderAuth {
    pub api_key: String,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub updated_at: Option<DateTime<Utc>>,
}

impl CustomProviderAuth {
...
impl AuthManager {
    ...
    /// Returns an auth snapshot appropriate for the requested provider. For providers that rely on
    /// the OpenAI login/token, this returns the cached auth. For custom providers, this constructs
    /// a temporary CodexAuth from the stored API key in auth.json (if present).
    pub fn auth_for_provider(
        ...
        match self.custom_provider_api_key(provider_id) {
            Ok(Some(api_key)) => Some(CodexAuth::from_api_key(&api_key)),
            Ok(None) => None,
            Err(err) => {
                warn!(
                    provider_id,
                    ?err,
                    "failed to load API key for custom provider"
                );
                None
            }
        }
    ...
    /// Return the stored API key for a custom provider if one exists.
    pub fn custom_provider_api_key(&self, provider_id: &str) -> std::io::Result<Option<String>> {
    ...
    /// Persist an API key for a custom provider. Creates auth.json if needed.
    pub fn store_custom_provider_api_key(
        &self,
        provider_id: &str,
        api_key: &str,
    ) -> std::io::Result<()> {
        ...
        let mut auth_dot_json = match try_read_auth_json(&auth_file) {
            Ok(auth) => auth,
            Err(err) if err.kind() == ErrorKind::NotFound => AuthDotJson::default(),
            Err(err) => return Err(err),
        };

        let map = auth_dot_json
            .custom_providers
            .get_or_insert_with(HashMap::new);
        map.insert(provider_id.to_string(), CustomProviderAuth::new(api_key));

        write_auth_json(&auth_file, &auth_dot_json)
    }

    /// Remove a stored API key for a custom provider, returning `true` if an entry was removed.
    pub fn delete_custom_provider_api_key(&self, provider_id: &str) -> std::io::Result<bool> {
        let auth_file = get_auth_file(&self.codex_home);
        let mut auth_dot_json = match try_read_auth_json(&auth_file) {
            Ok(auth) => auth,
        ...
        let removed = auth_dot_json
            .custom_providers
            .as_mut()
            .and_then(|map| map.remove(provider_id))
            .is_some();

        if removed {
            let should_clear = auth_dot_json
                .custom_providers
                .as_ref()
                .map(std::collections::HashMap::is_empty)
                .unwrap_or(false);
            if should_clear {
                auth_dot_json.custom_providers = None;
            }
            write_auth_json(&auth_file, &auth_dot_json)?;
        }
