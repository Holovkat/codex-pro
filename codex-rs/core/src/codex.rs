...
impl SessionConfiguration {
    pub(crate) fn apply(&self, updates: &SessionSettingsUpdate) -> Self {
        ...
        if let Some(provider_id) = updates.model_provider_id.clone() {
            next_configuration.provider_id = provider_id.clone();
            #[expect(
                clippy::map_clone,
                reason = "When only provider_id is overridden we must pull the downstream provider metadata without losing custom headers."
            )]
            #[expect(
                clippy::redundant_closure_for_method_calls,
                reason = "Closure keeps provider metadata lookup stable across upstream refactors."
            )]
            let provider = updates.model_provider.clone().or_else(|| {
                next_configuration
        ...
        } else if let Some(provider) = updates.model_provider.clone() {
            next_configuration.provider = provider;
        }
...
impl Session {
    fn make_turn_context(
        ...
        let model_family = find_family_for_model(&session_configuration.model)
            .unwrap_or_else(|| derive_default_model_family(&session_configuration.model));
        let mut per_turn_config = (*config).clone();
        per_turn_config.model_provider_id = session_configuration.provider_id.clone();
        per_turn_config.model_provider = provider.clone();
    ...
    async fn new(
        ...
        let auth_mode = auth_snapshot.as_ref().map(|a| a.mode);
        ...
        match sub.op.clone() {
            ...
            Op::OverrideTurnContext {
                ...
                let provider = provider_id
                    .as_ref()
                    .and_then(|id| config.model_providers.get(id).cloned());
            ...
            Op::MemoryPreviewDecision { accepted_ids } => {
                handlers::memory_preview_decision(&sess, accepted_ids).await;
            }
            Op::Shutdown => {
...
mod handlers {
    ...
    use crate::codex::format_memory_hits_message;
    ...
    pub async fn list_mcp_tools(sess: &Arc<Session>, config: &Arc<Config>, sub_id: String) {
        let config = Arc::clone(config);
        let sess_clone = Arc::clone(sess);

        tokio::spawn(async move {
            let tools = sess_clone.services.mcp_connection_manager.list_all_tools();
    ...
    pub async fn memory_preview_decision(sess: &Arc<Session>, accepted_ids: Vec<String>) {
        let Some(runtime) = sess.memory_runtime().await else {
            debug!("memory preview decision ignored; memory runtime disabled");
            return;
        };
        ...
        let Some(hits) = hits else {
            debug!("memory preview decision received without pending hits");
            return;
        };
