...
mod handlers {
                ...
                msg: EventMsg::GetHistoryEntryResponse(
                    crate::protocol::GetHistoryEntryResponseEvent {
                        ...
                        entry: entry_opt.map(|e| codex_protocol::message_history::HistoryEntry {
                            ...
                            message_line_number: e.message_line_number,
                            text: e.text,
                        }),
                    },
                ),
    ...
    pub async fn list_mcp_tools(sess: &Arc<Session>, config: &Arc<Config>, sub_id: String) {
        ...
        tokio::spawn(async move {
            let auth_statuses = compute_auth_statuses(&config);
            let tools = config.mcp_servers.clone();
            let resources = config.mcp_resources.clone().unwrap_or_default();
            let resource_templates = config
