...
const OPENAI_REVIEW_MODEL_SLUG: &str = "gpt-5-codex";
...
async fn spawn_review_thread(
    ...
    let mut model = config.review_model.clone();
    if !provider.requires_openai_auth && model == OPENAI_REVIEW_MODEL_SLUG {
        model = parent_turn_context.client.get_model();
    }
