...
impl Config {
    ...
    pub fn load_from_base_config_with_overrides(
        ...
        let mut model_provider_id = requested_model_provider_id
            .clone()
            .unwrap_or_else(|| "openai".to_string());

        let (model_provider, provider_missing) =
            match model_providers.get(&model_provider_id).cloned() {
                Some(provider) => (provider, false),
                None => {
                    let fallback_id = "openai".to_string();
                    let fallback_provider =
                        model_providers.get(&fallback_id).cloned().ok_or_else(|| {
                            std::io::Error::new(
                                std::io::ErrorKind::NotFound,
                                "Fallback OpenAI provider not available",
                            )
                        })?;
                    if let Some(missing_id) = requested_model_provider_id {
                        tracing::warn!(
                            missing_provider = %missing_id,
                            "Model provider removed from settings; falling back to `openai`",
                        );
                    } else {
                        tracing::warn!(
                            "Model provider missing from settings; falling back to `openai`"
                        );
                    }
                    model_provider_id = fallback_id;
        ...
        let model_override_provided = model.is_some();
        ...
        if provider_missing && !model_override_provided {
            let fallback_model = default_model();
            tracing::info!(
                "Resetting model to default `{fallback}` after provider fallback",
        ...
        let needs_minimal_prompt =
            provider_needs_minimal_prompt(&config.model_provider_id, &config.model_provider);
        tracing::debug!(
            provider = %config.model_provider_id,
            base_url = ?config.model_provider.base_url,
            needs_minimal_prompt,
            "evaluated coding plan prompt flag"
        );
        if needs_minimal_prompt {
            tracing::debug!(
                provider = %config.model_provider_id,
                "applying minimal coding plan instructions"
            );
            config.base_instructions = Some(MINIMAL_CODING_PLAN_BASE_INSTRUCTIONS.to_string());
...
const MINIMAL_CODING_PLAN_BASE_INSTRUCTIONS: &str =
    "You are a concise coding assistant. Respond directly to the user's requests.";

fn provider_needs_minimal_prompt(provider_id: &str, provider: &ModelProviderInfo) -> bool {
