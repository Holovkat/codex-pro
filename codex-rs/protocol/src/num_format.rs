...
thread_local! {
    static FORMATTER: RefCell<Option<DecimalFormatter>> = const { RefCell::new(None) };
}

fn with_formatter<F, R>(f: F) -> R
where
    F: FnOnce(&DecimalFormatter) -> R,
{
    FORMATTER.with(|formatter| {
        ...
        if let Some(formatter) = formatter.as_ref() {
            ...
            unreachable!("formatter initialisation failed");
        }
    })
}
