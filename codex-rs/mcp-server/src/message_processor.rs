...
use crate::codex_tool_config::CodexCommandCallParam;
use crate::codex_tool_config::CodexToolCallParam;
use crate::codex_tool_config::CodexToolCallReplyParam;
use crate::codex_tool_config::create_tool_for_codex_command_call_param;
...
impl MessageProcessor {
    ...
    async fn handle_list_tools(
        ...
        let result = ListToolsResult {
            tools: vec![
                ...
                create_tool_for_codex_command_call_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(
        ...
        match name.as_str() {
            ...
            "codex-command" => self.handle_command_tool(id, arguments).await,
            _ => {
    ...
    async fn handle_command_tool(&self, id: RequestId, arguments: Option<serde_json::Value>) {
        let payload = match arguments {
            Some(json_val) => match serde_json::from_value::<CodexCommandCallParam>(json_val) {
                Ok(payload) => payload,
                Err(err) => {
                    let result = CallToolResult {
                        content: vec![ContentBlock::TextContent(TextContent {
                            r#type: "text".to_string(),
                            text: format!("Failed to parse command arguments: {err}"),
                            annotations: None,
                        })],
                        is_error: Some(true),
                        structured_content: None,
            ...
            None => {
                let result = CallToolResult {
                    content: vec![ContentBlock::TextContent(TextContent {
                        r#type: "text".to_string(),
                        text: "Missing arguments for codex-command tool-call; expected `{\"command\": ..., \"args\": [...]}`.".to_string(),
                        annotations: None,
                    })],
                    is_error: Some(true),
                    structured_content: None,
        ...
        let CodexCommandCallParam { command, args } = payload;
        let ctx = self.command_ctx.clone();
        let registry = self.command_registry.clone();
        let response = match registry.run(&ctx, &command, &args) {
            Ok(result) => self.command_result_to_call_tool(result),
            Err(err) => CallToolResult {
                content: vec![ContentBlock::TextContent(TextContent {
                    r#type: "text".to_string(),
                    text: format!("Command `{command}` failed: {err}"),
                    annotations: None,
                })],
                is_error: Some(true),
                structured_content: None,
        ...
        self.send_response::<mcp_types::CallToolRequest>(id, response)
            .await;
    }

    fn command_result_to_call_tool(&self, result: CommandResult) -> CallToolResult {
        match result {
            CommandResult::Unit => CallToolResult {
                content: vec![ContentBlock::TextContent(TextContent {
                    r#type: "text".to_string(),
            ...
            CommandResult::Text(text) => CallToolResult {
                content: vec![ContentBlock::TextContent(TextContent {
                    r#type: "text".to_string(),
                    text,
                    annotations: None,
            ...
            CommandResult::Json(value) => {
                let pretty = serde_json::to_string_pretty(&value)
                    .unwrap_or_else(|_| "<failed to render JSON>".to_string());
                CallToolResult {
                    content: vec![ContentBlock::TextContent(TextContent {
                        r#type: "text".to_string(),
                        text: pretty,
