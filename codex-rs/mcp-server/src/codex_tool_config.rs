...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, Default)]
pub struct CodexCommandCallParam {
    /// Command name registered in the shared command registry.
    pub command: String,
...
/// Builds a `Tool` definition for invoking commands from the shared registry.
pub(crate) fn create_tool_for_codex_command_call_param() -> Tool {
    let schema = SchemaSettings::draft2019_09()
        .with(|s| {
            s.inline_subschemas = true;
            s.option_add_null_type = false;
        ...
        .into_generator()
        .into_root_schema_for::<CodexCommandCallParam>();

    #[expect(clippy::expect_used)]
    let schema_value =
        serde_json::to_value(&schema).expect("Codex command tool schema should serialise to JSON");
    ...
    Tool {
        name: "codex-command".to_string(),
        title: Some("Codex Command".to_string()),
        input_schema: tool_input_schema,
        output_schema: None,
        description: Some(
            "Run a Codex CLI command (e.g., models.list) using the shared command registry."
                .to_string(),
        ),
        annotations: None,
    }
