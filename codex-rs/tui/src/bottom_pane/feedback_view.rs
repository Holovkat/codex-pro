...
impl FeedbackView {
    ...
    fn selection_params(
        ...
        let upload_action_thread_id = thread_id.clone();
        let upload_action: SelectionAction =
            Box::new(
                move |tx: &AppEventSender| match snapshot.upload_to_sentry() {
                    Ok(()) => {
                        let issue_url = format!(
                            "{BASE_ISSUE_URL}&steps=Uploaded%20thread:%20{upload_action_thread_id}",
                        );
                        ...
                        Line::from(vec![
                            ...
                            upload_action_thread_id.clone().bold(),
...
impl FeedbackHeader {
    ...
    fn lines(&self) -> Vec<Line<'static>> {
        vec![
            ...
            "".into(),
            Line::from("You can review the exact content of the logs before they’re uploaded at:"),
