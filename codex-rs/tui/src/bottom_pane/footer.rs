...
pub(crate) fn footer_height(props: &FooterProps) -> u16 {
    footer_lines(props.clone()).len() as u16
}
...
fn footer_lines(props: FooterProps) -> Vec<Line<'static>> {
    let mut lines = match props.mode {
        ...
        FooterMode::ShortcutSummary => {
            let mut line =
                context_and_limits_line(props.context_window_percent, &props.rate_limit_summaries);
            line.extend(vec![
                Span::from(" · ").dim(),
        ...
        FooterMode::ContextOnly => vec![context_and_limits_line(
            props.context_window_percent,
            &props.rate_limit_summaries,
        )],
    };
