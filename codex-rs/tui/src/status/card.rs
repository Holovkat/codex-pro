...
use super::helpers::compose_prompts_list;
...
impl StatusHistoryCell {
    fn new(
        ...
        let prompt_summary = compose_prompts_list(config);
...
impl HistoryCell for StatusHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some((first_prompt, rest_prompts)) = self.prompt_summary.split_first() {
            lines.push(formatter.line("Prompts", vec![Span::from(first_prompt.clone())]));
            for entry in rest_prompts {
                lines.push(formatter.continuation(vec![Span::from(entry.clone())]));
            }
        }
