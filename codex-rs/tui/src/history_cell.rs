...
#[derive(Debug)]
pub(crate) struct SearchResultsCell {
    query: String,
    confidence_percent: u8,
    entries: Vec<SearchResultEntry>,
}

pub(crate) fn new_search_results(
    query: String,
    confidence: f32,
    hits: Vec<QueryHit>,
) -> Box<dyn HistoryCell> {
    ...
    let entries = hits
        ...
        .map(|hit| SearchResultEntry {
            rank: hit.rank,
            score_percent: ((hit.score * 100.0).round()).clamp(0.0, 100.0) as u8,
...
fn clean_snippet(snippet: &str) -> Option<String> {
    let collapsed = snippet.split_whitespace().collect::<Vec<_>>().join(" ");
    if collapsed.is_empty() {
        None
    } else if collapsed.len() > 160 {
        let mut truncated = collapsed.chars().take(157).collect::<String>();
        truncated.push_str("...");
        Some(truncated)
    } else {
        Some(collapsed)
...
impl HistoryCell for SearchResultsCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let header = format!(
            ...
            self.query, self.confidence_percent
        );
        lines.push(Line::from(header.bold()));

        if self.entries.is_empty() {
            lines.push(Line::from("No indexed matches met the threshold.".dim()));
            return lines;
        }
        ...
        for (idx, entry) in self.entries.iter().enumerate() {
            ...
            let spans = vec![
                format!("  {}.", entry.rank).dim(),
                " ".into(),
                format!("{:>3}%", entry.score_percent).cyan(),
                " ".into(),
                entry.file_path.clone().cyan(),
                format!(":{}-{}", entry.start_line, entry.end_line).dim(),
            ];
            lines.push(Line::from(spans));

            if let Some(snippet) = entry.snippet.as_ref() {
                let wrap_width = width.saturating_sub(4).max(1) as usize;
                let snippet_line = Line::from(snippet.clone().dim());
                let wrapped = word_wrap_line(&snippet_line, RtOptions::new(wrap_width));
    ...
    fn search_results_cell_formats_hits() {
        ...
        let hits = vec![
            ...
            QueryHit {
                ...
                start_line: 20,
                end_line: 25,
                snippet: "let value = compute();".to_string(),
            },
        ];

        let cell = new_search_results("main".to_string(), 0.6, hits);
        let rendered = render_lines(&cell.display_lines(60));
        assert_eq!(
            rendered,
            vec![
                "Search results for \"main\" (confidence ≥ 60%)".to_string(),
                "  1.  92% src/lib.rs:10-14".to_string(),
                "    fn main() {}".to_string(),
                String::new(),
