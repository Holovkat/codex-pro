...
This will make a local commit on top of `main` with `version` set to `$VERSION` in `codex-rs/Cargo.toml` (main now tracks the released upstream version, currently `0.50.0`).
